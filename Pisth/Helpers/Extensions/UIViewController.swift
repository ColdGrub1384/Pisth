...
extension UIViewController {
    ...
    static var connectionInfo: ConnectionInformationTableViewController {
        return (UIStoryboard(name: "Connection Info", bundle: Bundle.main).instantiateInitialViewController() as? ConnectionInformationTableViewController) ?? ConnectionInformationTableViewController(style: .plain)
    }
    ...
    /// Returns the Snippet info view controller initialised from nib.
    static var snippetInfo: SnippetInfoViewController {
        return (Bundle.main.loadNibNamed("Snippet Info", owner: nil, options: nil)?[0] as? SnippetInfoViewController) ?? SnippetInfoViewController()
    }
}
