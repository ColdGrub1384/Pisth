...
class DirectoryCollectionViewController: UICollectionViewController, LocalDirectoryCollectionViewControllerDelegate, DirectoryCollectionViewControllerDelegate, UIDocumentPickerDelegate, UICollectionViewDragDelegate, UICollectionViewDropDelegate, SKStoreProductViewControllerDelegate, PanelContentDelegate {
    ...
    /// The floating panel containing the `SnippetsViewController`.
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        let contentViewController = SnippetsViewController.makeViewController(connection: connection, directory: directory)
        contentViewController.expansionHandler = {
            self.floatingPanel?.move(to: .full, animated: true)
        }
        contentViewController.collapsionHandler = {
            self.floatingPanel?.move(to: .tip, animated: true)
        }
        floatingPanel?.set(contentViewController: contentViewController)
        floatingPanel?.track(scrollView: contentViewController.tableView)
        floatingPanel?.addPanel(toParent: ContentViewController.shared)
